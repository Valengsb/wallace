...
test_that("output type checks", {
  ...
  # a data frame
  expect_is(bioclimAlg$evalTblBins, "data.frame")
  # a raster Stack
  ...
  expect_equal(nrow(bioclimAlg$evalTbl), 1)
  # columns name in the evaluation table are right
  expect_equal(colnames(bioclimAlg$evalTbl),c("avg.test.AUC", "var.test.AUC", "avg.diff.AUC", 
                                              "var.diff.AUC", "avg.test.orMTP", "var.test.orMTP", 
                                              "avg.test.or10pct", "var.test.or10pct"))
  # rows name in the evaluation table are right
  expect_equal(rownames(bioclimAlg$evalTbl), "BIOCLIM")
...
test_that("output data checks", {
  ...
  expect_false(FALSE %in% ((bioclimAlg$evalTbl[,c("avg.test.AUC", "var.test.AUC", "avg.diff.AUC", 
                                                "var.diff.AUC")])<1 | 
